...
public interface StarMapper {
    ...
    //查询用户的点赞状态
    int getStatus(@Param("userId")int userId,@Param("postId")int postId);
    //未点赞，改为点赞状态
    int updateStatusTrue(@Param("userId") int userId, @Param("postId") int postId);
    //已经点赞，改为未点赞状态
    int updateStatusFalse(@Param("userId")int userId,@Param("postId") int postId);
    //点赞前先将关系加入该表
    int addRelateOfZan(@Param("userId")int userId,@Param("postId") int postId);
    //判断该关系是否已经存在
